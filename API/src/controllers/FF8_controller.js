...
exports.getAllData = (request, response) => {
  fs.readFile(chemin, (err, data) => {
    if (err) {
      ...
      response.status(200).json(JSON.parse(data).FF8);
    }
  });
}
...
exports.getDataById = (request, response) => {
  fs.readFile(chemin, (err, data) => {
    if (err) {
      ...
      const dataById = jsonData.FF8.find(
...
exports.createData = (request, response) => {
  fs.readFile(chemin, (err, data) => {
    if (err) {
      ...
      let lastId = existingData.FF8.findLast((obj) => obj.id); // cherche dernier objet;
      let newTabLine = { id: lastId.id + 1, name: request.body.name };
      let oldName = existingData.FF8.find(
      ...
      if (oldName) {
        ...
        existingData.FF8.push(newTabLine);
...
exports.updateData = (request, response) => {
  fs.readFile(chemin, (err, data) => {
    if (err) {
      ...
      const existingData = JSON.parse(data);
      const dataById = existingData.FF8.find(
        (obj) => obj.id === parseInt(request.params.id)
      );
      if (!dataById) {
        response.status(404).json({
  ...
  fs.readFile(chemin, (err, data) => {
    if (err) {
      ...
      if (!dataById) {
        ...
        existingData.FF8 = existingData.FF8.filter(
          (obj) => obj.id != parseInt(request.params.id)
        );
        fs.writeFile(chemin, JSON.stringify(existingData), (writeErr) => {
          if (writeErr) {
